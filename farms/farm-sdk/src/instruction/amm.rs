...
impl AmmInstruction {
    ...
    pub const ADD_LIQUIDITY_LEN: usize = 25;
    pub const REMOVE_LIQUIDITY_LEN: usize = 25;
    pub const SWAP_LEN: usize = 25;
    pub const STAKE_LEN: usize = 9;
    pub const UNSTAKE_LEN: usize = 9;
    pub const HARVEST_LEN: usize = 1;
    pub const INITVAULT_LEN: usize = 1;
    pub const VALCRANK_LEN: usize = 9;
    ...
    pub fn pack(&self, output: &mut [u8]) -> Result<usize, ProgramError> {
        match self {
            ...
            Self::InitVault { .. } => self.pack_init_vault(output),
            Self::ValCrank { .. } => self.pack_crank1(output),
    ...
    pub fn unpack(input: &[u8]) -> Result<AmmInstruction, ProgramError> {
        ...
        match instruction_type {
            ...
            AmmInstructionType::InitVault => AmmInstruction::unpack_init_vault(input),
            AmmInstructionType::ValCrank => AmmInstruction::unpack_crank1(input),
    ...
    fn pack_add_liquidity(&self, output: &mut [u8]) -> Result<usize, ProgramError> {
        ...
        } = self
        {
            ...
            let (instruction_type_pack, max_token_a_amount_pack, max_token_b_amount_pack,pool_token_amount_pack) =
            ...
            *pool_token_amount_pack = pool_token_amount.to_le_bytes();
    ...
    fn pack_remove_liquidity(&self, output: &mut [u8]) -> Result<usize, ProgramError> {
        ...
        if let AmmInstruction::RemoveLiquidity { pool_token_amount,
            minimum_token_a_amount,
            minimum_token_b_amount } 
            = self {
            let output = array_mut_ref![output, 0, AmmInstruction::REMOVE_LIQUIDITY_LEN];
            let (instruction_type_pack, pool_token_amount_pack,minimum_token_a_amount_pack,minimum_token_b_amount_pack) = mut_array_refs![output, 1, 8, 8, 8];
            instruction_type_pack[0] = AmmInstructionType::RemoveLiquidity as u8;
            *pool_token_amount_pack = pool_token_amount.to_le_bytes();
            *minimum_token_a_amount_pack = minimum_token_a_amount.to_le_bytes();
            *minimum_token_b_amount_pack = minimum_token_b_amount.to_le_bytes();
    ...
    fn pack_init_vault(&self, output: &mut [u8]) -> Result<usize, ProgramError> {
        check_data_len(output, AmmInstruction::INITVAULT_LEN)?;
        if let AmmInstruction::InitVault = self {
            let instruction_type_pack = array_mut_ref![output, 0, 1];
            instruction_type_pack[0] = AmmInstructionType::InitVault as u8;
            Ok(AmmInstruction::INITVAULT_LEN)
        } else {
            Err(ProgramError::InvalidInstructionData)
        }
    }

    fn pack_crank1(&self, output: &mut [u8]) -> Result<usize, ProgramError> {
        check_data_len(output, AmmInstruction::VALCRANK_LEN)?;
        if let AmmInstruction::ValCrank { step } = self {
            let output = array_mut_ref![output, 0, AmmInstruction::VALCRANK_LEN];
            let (instruction_type_pack, amount_pack) = mut_array_refs![output, 1, 8];
            instruction_type_pack[0] = AmmInstructionType::Unstake as u8;
            *amount_pack = step.to_le_bytes();
            Ok(AmmInstruction::VALCRANK_LEN)
        } else {
            Err(ProgramError::InvalidInstructionData)
        }
    }
    ...
    fn unpack_add_liquidity(input: &[u8]) -> Result<AmmInstruction, ProgramError> {
        ...
        let (max_token_a_amount, max_token_b_amount,pool_token_amount) = array_refs![input, 8, 8, 8];
        //msg!("{} {}",max_token_a_amount,max_token_b_amount);
    ...
    fn unpack_remove_liquidity(input: &[u8]) -> Result<AmmInstruction, ProgramError> {
        ...
        let input = array_ref![input, 1, AmmInstruction::ADD_LIQUIDITY_LEN - 1];
        #[allow(clippy::ptr_offset_with_cast)]
        let(pool_token_amount,minimum_token_a_amount,minimum_token_b_amount)=array_refs![input, 8 , 8, 8];
    ...
    fn unpack_init_vault(input: &[u8]) -> Result<AmmInstruction, ProgramError> {
        check_data_len(input, AmmInstruction::INITVAULT_LEN)?;
        Ok(Self::InitVault)
    }
    fn unpack_crank1(input: &[u8]) -> Result<AmmInstruction, ProgramError> {
        check_data_len(input, AmmInstruction::VALCRANK_LEN)?;
        Ok(Self::ValCrank {
...
impl std::fmt::Display for AmmInstructionType {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match *self {
            ...
            AmmInstructionType::InitVault => write!(f, "InitVault"),
            AmmInstructionType::ValCrank => write!(f, "ValCrank"),
