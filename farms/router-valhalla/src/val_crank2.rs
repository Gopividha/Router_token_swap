...
pub fn val_crank2(accounts: &[AccountInfo]) -> ProgramResult {
        ...
        ] = accounts
    {
        ...
        let pda_prefix  = "Val_Reaward_VaultPDA";
        let pda_seed = &[
            pda_prefix.as_bytes(),
            reward_mint.key.as_ref(),
        ];
                
        let (pda_vault, nonce) = Pubkey::find_program_address(pda_seed,program_id);

        msg!("pda {}", pda_vault);
        
        // get PDA's associated account for reward
        let pda_reward_token_account = spl_associated_token_account::get_associated_token_address;(
            pda_reward,
            reward_mint,
        )?;


        let instruction = instruction::harvest(
            user_account.key,
    ...
    Ok(())
        ...
        // harvest -- pda
        // pda -- swap acc to pool ratios
        // pool ke a and b tokens
        // addliquidity
        // stake
